...
class MicrophonePublisher(Node):
    def __init__(self):
        ...
        self.timer_ = self.create_timer(0.1, self.publish_microphone_data)
        ...
        # Listar os dispositivos de entrada (microfones)
        print("Dispositivos de entrada (microfones):")
        for i in range(self.p.get_device_count()):
            device_info = self.p.get_device_info_by_index(i)
            if device_info["maxInputChannels"] > 0:
              print(f"ID: {device_info['index']}, Nome: {device_info['name']}")
